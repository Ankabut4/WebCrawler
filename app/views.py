...
def json_stats(request):
    json_data = open(djangoSettings.STATIC_ROOT+'/app/json/stats.json')   
    data1 = json.load(json_data)
    return JsonResponse(data1, safe=False)
...
def ajax_scan_sites(request):
    ...
    try:
        ...
        for page_id in range(1,500000):
            ...
            for row in soup.findAll('div', {"class": 'record'}):
                ...
                if rec.county not in counties:
                    counties.append(rec.county)
                    stats[rec.county]=dict(total=1,pending=1, not_found=0, found=0)
                else:
                    stats[rec.county]=dict(total=stats[rec.county]['total']+1,pending=stats[rec.county]['pending']+1, not_found=0, found=0)
    except:
    ...
    with open(djangoSettings.STATIC_ROOT+'/app/json/status.json', 'w') as json_file1:
        ...
        json_file1.close()
    with open(djangoSettings.STATIC_ROOT+'/app/json/counties.json', 'w') as json_file2:
        json.dump(counties, json_file2)
        json_file2.close()
    with open(djangoSettings.STATIC_ROOT+'/app/json/stats.json', 'w') as json_file3:
        json.dump(OrderedDict(sorted(stats.items(), key=lambda t: t[0])), json_file3)
        json_file3.close()
